...
#if defined(USE_FIXED_BASE_SM) || defined(USE_DOUBLE_SM)
...
{ // Fixed-base scalar multiplication Q = k*P, where the base point P is passed through P_Table which contains multiples of P.
  ...
  // with (bitlength(order)+w*v) digits.
    ...
    point_precomp_t* P_table = (point_precomp_t*)P_Table;
    point_extproj_t R;
    point_precomp_t S;
    int i, ii;
    
    modulo_order(k, k, curve);                                  // k = k mod (order) 
    conversion_to_odd(k, (digit_t*)temp, curve);                // Converting scalar to odd using the prime subgroup order
...
{ // Allocates memory dynamically for the precomputation table used during fixed-base scalar multiplications.
  // This function must be called before using ecc_precomp_fixed(), which generates a precomputed table with v*2(w-1) = NPOINTS_FIXEDBASE points.

    return (digit_t*)calloc(NPOINTS_FIXEDBASE, sizeof(point_precomp_t));
}


bool ecc_precomp_fixed(point_t P, digit_t* table, bool clear_cofactor, PCurveStruct curve)
{ // Generation of the precomputation table used by the fixed-base scalar multiplication function ecc_mul_fixed(). 
  ...
  // This function performs point validation and (if selected) cofactor clearing.
    point_t S;
    point_precomp_t *Table = (point_precomp_t*)table;
...
{ // Double scalar multiplication R = k*P + l*Q, where the base point P is passed through P_Table which contains multiples of P, Phi(P), Psi(P) and Phi(Psi(P)).
  // Inputs: precomputed table "P_Table" containing 4*NPOINTS_DOUBLEMUL_WP points that are generated by 4 calls to function ecc_precomp_double(),
  //         each set corresponding to multiples of P, Phi(P), Psi(P) and Phi(Psi(P)), respectively,  
  //         point Q in affine coordinates,
  //         Scalars "k" and "l" in [0, 2^256-1].
  //         FourQ structure "curve".
  // Output: R = k*P + l*Q in affine coordinates (x,y).
  // The function uses wNAF with interleaving.
    ...
    point_precomp_t V, *P_table = (point_precomp_t*)P_Table;
    ...
    point_setup(Q, Q1);                                        // Clear most significant bit of input coordinates and convert to representation (X,Y,1,Ta,Tb)
    
    if (ecc_point_validate(Q1, curve) == false) {              // Check if point lies on the curve
        return false;
    }
    ...
    ecc_precomp_double_internal(Q1, Q_table1, NPOINTS_DOUBLEMUL_WQ, curve);    // Precomputation
    ecc_precomp_double_internal(Q2, Q_table2, NPOINTS_DOUBLEMUL_WQ, curve); 
    ecc_precomp_double_internal(Q3, Q_table3, NPOINTS_DOUBLEMUL_WQ, curve); 
    ecc_precomp_double_internal(Q4, Q_table4, NPOINTS_DOUBLEMUL_WQ, curve); 
    ...
    {   
        ...
        if (digits_k1[i] < 0) {
            ...
            eccneg_precomp(P_table[position], V);              // Load and negate V = (X_V,Y_V,Z_V,Td_V) <- -(x+y,y-x,2dt) from a point in the precomputed table 
            ...
            eccmadd(P_table[position], T);                     // T = T+V = (X_T,Y_T,Z_T,Ta_T,Tb_T) = (X_T,Y_T,Z_T,Ta_T,Tb_T) + (X_V,Y_V,Z_V,Td_V) 
        }
        if (digits_k2[i] < 0) {
            position = (-digits_k2[i])/2;                      
            eccneg_precomp(P_table[NPOINTS_DOUBLEMUL_WP+position], V);              
            eccmadd(V, T);                              
        } else if (digits_k2[i] > 0) {            
            position = (digits_k2[i])/2;                       
            eccmadd(P_table[NPOINTS_DOUBLEMUL_WP+position], T);               
        }
        if (digits_k3[i] < 0) {
            position = (-digits_k3[i])/2;                      
            eccneg_precomp(P_table[2*NPOINTS_DOUBLEMUL_WP+position], V);              
            eccmadd(V, T);                              
        } else if (digits_k3[i] > 0) {            
            position = (digits_k3[i])/2;                       
            eccmadd(P_table[2*NPOINTS_DOUBLEMUL_WP+position], T);               
        }
        if (digits_k4[i] < 0) {
            position = (-digits_k4[i])/2;                      
            eccneg_precomp(P_table[3*NPOINTS_DOUBLEMUL_WP+position], V);              
            eccmadd(V, T);                              
        } else if (digits_k4[i] > 0) {            
            position = (digits_k4[i])/2;                       
            eccmadd(P_table[3*NPOINTS_DOUBLEMUL_WP+position], T);               
        }
    }
...
void ecc_precomp_double_internal(point_extproj_t P, point_extproj_precomp_t* Table, unsigned int npoints, PCurveStruct curve)
{ // Generation of the precomputation table used internally by the double scalar multiplication function ecc_mul_double().  
  ...
  // Output: Table containing multiples of the base point P using representation (X+Y,Y-X,2Z,2dT).
    ...
    R1_to_R2(P, Table[0], curve);              // Precomputed point Table[0] = P in coordinates (X+Y,Y-X,2Z,2dT)
    eccdouble_ni(P);                           // A = 2*P in (X,Y,Z,Ta,Tb)
    ...
    for (i = 1; i < npoints; i++) {
        ...
        R1_to_R2(Q, Table[i], curve);          // Converting from (X,Y,Z,Ta,Tb) to (X+Y,Y-X,2Z,2dT)
    }
...
{ // Allocates memory dynamically for the precomputation table used during double scalar multiplications.
  // This function must be called before using ecc_precomp_double(), which generates a precomputed table with NPOINTS_DOUBLEMUL_WP points.

    return (digit_t*)calloc(4*NPOINTS_DOUBLEMUL_WP, sizeof(point_precomp_t));
}
...
{ // Generation of the precomputation table used by the double scalar multiplication function ecc_mul_double(). 
  // Inputs: point P in affine representation (x,y),
  //         table with storage for NPOINTS_DOUBLEBASE_WP points. Allocation for this space must be done by calling ecc_allocate_precomp_double(), 
  //         FourQ structure "curve".
  // Output: table containing multiples of the base point P using affine coordinates with representation (x+y,y-x,2dt). 
  // This function performs point validation.
    point_precomp_t *Table = (point_precomp_t*)table;
    point_extproj_t P1, P2, P3, P4;
    point_extproj_precomp_t T[4*NPOINTS_DOUBLEMUL_WP]; 
    unsigned int i;
    
    point_setup(P, P1);                                  // Clear most significant bit of input coordinates and convert to representation (X,Y,1,Ta,Tb)

    if (ecc_point_validate(P1, curve) == false) {        // Check if point lies on the curve
        return false;
    }
    ...
    ecc_precomp_double_internal(P1, T, NPOINTS_DOUBLEMUL_WP, curve); 
    ecc_precomp_double_internal(P2, T+NPOINTS_DOUBLEMUL_WP, NPOINTS_DOUBLEMUL_WP, curve); 
    ecc_precomp_double_internal(P3, T+2*NPOINTS_DOUBLEMUL_WP, NPOINTS_DOUBLEMUL_WP, curve); 
    ecc_precomp_double_internal(P4, T+3*NPOINTS_DOUBLEMUL_WP, NPOINTS_DOUBLEMUL_WP, curve); 
           
    for (i = 0; i < 4*NPOINTS_DOUBLEMUL_WP; i++) {
        fp2div1271(T[i]->z2);
        fp2inv1271(T[i]->z2);
        fp2mul1271(T[i]->xy, T[i]->z2, Table[i]->xy);
        fp2mul1271(T[i]->yx, T[i]->z2, Table[i]->yx);
        fp2mul1271(T[i]->t2, T[i]->z2, Table[i]->t2);
    }
