...
static uint64_t Montgomery_Rprime[4] = { 0xC81DB8795FF3D621, 0x173EA5AAEA6B387D, 0x3D01B7C72136F61C, 0x0006A5F16AC8F9D3 };  
static uint64_t Montgomery_rprime[4] = { 0xE12FE5F079BC3929, 0xD75E78B8D1FCDCF3, 0xBCE409ED76B5DB21, 0xF32702FDAFC1C074 };   
...
{ // Reduction modulo the order using Montgomery arithmetic
  ...
  // c = ma*1*Montgomery_Rprime^(-1) mod r, where ma,c in [0, r-1], ma,c,r < 2^256
    ...
    one[0] = 1;
    Montgomery_multiply(a, (digit_t*)&Montgomery_Rprime, ma, (digit_t*)&curve->order);
    Montgomery_multiply(ma, one, c, (digit_t*)&curve->order);
}
...
{ // Convert scalar to odd if even using the prime subgroup order r
    digit_t mask;
    digit_t* order = (digit_t*)curve->order;
    unsigned int i, carry = 0;
...
static __inline void fpdiv1271(felm_t a)
