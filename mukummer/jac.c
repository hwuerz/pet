...
void jacobian_scalarmult(uint32 *r, const uint32 *n, const uint32 *p, const uint32 *xpw)
{
    ...
    xUNWRAP(xp, xpw);		   
	crypto_scalarmult(xqxpq, n, xpw, l);	//TODO: two lines cause segfault (keygen) (sign) (at tests with test_pet.c)
...
// xqxpq = [x(Q), x(Q+P)]
void recoverFast(uint32 *r, const uint32 *p, const uint32 *xp, const uint32 *xq, const uint32 *xpq) //TODO: might cause segfault (keygen)
{
