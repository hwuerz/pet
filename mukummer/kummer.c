...
void crypto_scalarmult(uint32 *q, const uint32 *n, const uint32 *p, const int l) //TODO: Causes segfault (keygen)
{
  int i;
  int bit;
  int prevbit;
  ...
  for(i=l;i>=0;i--)
  {
    ...
    gfe_neg(q+16, q+16);
    cswap4x(q, q+16, swap);
    xDBLADD(q, q+16, q, q+16, p); //TODO: Causes Segfault (test_pet.c) - in context of the for loop AND even for unrolled context.
  }
  gfe_neg(q, q);
  cswap4x(q,q+16,bit);
}
...
{
  hadamard(w4, t);		//TODO: At least those two lines (together and alone) cause segfaults
  hadamard(w0, z);
  ...
  hadamard(w4, w4);		//TODO: Those two lines (in every combination) cause segfaults as well (for all above lines removed)
  hadamard(w0, w0);
