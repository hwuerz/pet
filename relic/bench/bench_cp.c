...
static void rsa(void) {
	rsa_t pub, prv;
	// in -> Input message 16 Byte
	// new -> ???
	// h -> Hash of the input message
    // MD_LEN -> Laenge des Hash: 32 Byte (=256Bit) fuer SHA256
	// out -> [1024 / 8 + 1] = [128+1] = 129 Bytes
    // RELIC_BN_BITS = Schlüssellaenge
	uint8_t in[140], new[140], h[MD_LEN], out[RELIC_BN_BITS / 8 + 1];
	int out_len, new_len;

	rsa_null(pub); // public key
	rsa_null(prv); // private key
